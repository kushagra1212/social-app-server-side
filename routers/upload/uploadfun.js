...
const uploadBaseUrl = process.env.BASEURL;
const getImageToken = async (originalname) => {
  try {
    const res = await axios.get(`${uploadBaseUrl}${originalname}`);
    if (res.data) {
...
module.exports.updateuser = async (req, res) => {
  ...
  if (!req.file) {
    ...
    blobWriter.on('finish', async () => {
      ...
      try {
        const user = await User.findById({ _id });
        user.profilepic = `${uploadBaseUrl}${req.file.originalname}?alt=media&token=${token}`;
        user.email = email;
        user.username = username;
        user.bio = bio;
        try {
